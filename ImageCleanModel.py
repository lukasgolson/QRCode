...
# Set global precision policy to mixed precision
...
def create_generator(input_shape):
    ...
    # Set final layer to float32 for numerical stability
...
def create_discriminator(input_shape):
    ...
    # Set output layer to float32 for stability
...
def train_model(resolution=256, epochs=100, batch_size=32, jit=False):
    ...
    val_dataset = Dataset.create_dataset(paired=True, target_size=(resolution, resolution), batch_size=batch_size, noisiest_epoch=50, batches_per_epoch=1)

    callbacks = [
        TensorBoard(log_dir="logs/fit/ImageCleanModel/" + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), profile_batch=0),
    ...
    gen_optimizer = tf.keras.mixed_precision.LossScaleOptimizer(tf.keras.optimizers.Adam(learning_rate=1e-4, beta_1=0.0, beta_2=0.9))
    disc_optimizer = tf.keras.mixed_precision.LossScaleOptimizer(tf.keras.optimizers.Adam(learning_rate=1e-4, beta_1=0.0, beta_2=0.9))
